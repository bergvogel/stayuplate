...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="breadcrumb-trail breadcrumbs" xmlns:v="http://rdf.data-vocabulary.org/#">';
			...
			if ( 1 < count( $this->items ) )
				array_unshift( $this->items, '<span class="trail-begin" typeof="v:Breadcrumb">' . array_shift( $this->items ) . '</span>' );

			/* Adds the 'trail-end' class around last item. */
			array_push( $this->items, '<span class="trail-end" typeof="v:Breadcrumb"><span property="v:title">' . array_pop( $this->items ) . '</span></span>' );
	...
	public function do_network_home_link() {
		if ( is_multisite() && !is_main_site() && true === $this->args['network'] )
			$this->items[] = '<a href="' . network_home_url() . '" title="' . esc_attr( $this->args['labels']['home'] ) . '" rel="v:url" property="v:title">' . $this->args['labels']['home'] . '</a>';
	}
	...
	public function do_site_home_link() {
		...
		$rel   = ( is_multisite() && !is_main_site() && true === $this->args['network'] ) ? '' : ' rel="v:url" property="v:title"';
	...
	public function do_posts_page_items() {
		...
		if ( is_paged() )
			$this->items[]  = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $title ) . '" rel="v:url" property="v:title">' . $title . '</a>';
	...
	public function do_singular_items() {
		...
		if ( $post_title = single_post_title( '', false ) ) {
			...
			if ( 1 < get_query_var( 'page' ) )
				$this->items[] = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $post_title ) . '" rel="v:url" property="v:title">' . $post_title . '</a>';
	...
	public function do_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			$parents[] = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '" rel="v:url" property="v:title">' . get_the_title( $post_id ) . '</a>';
	...
	public function do_post_hierarchy( $post_id ) {
		...
		if ( !empty( $post_type_object->has_archive ) ) {
			...
			$this->items[] = '<a href="' . get_post_type_archive_link( $post_type ) . '" rel="v:url" property="v:title">' . $label . '</a>';
		}
	}
	...
	public function do_term_archive_items() {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( !empty( $post_types ) ) {
							...
							$this->items[] = '<a href="' . get_post_type_archive_link( $post_type_object->name ) . '" title="' . esc_attr( $label ) . '" rel="v:url" property="v:title">' . $label . '</a>';
		...
		if ( is_paged() )
			$this->items[] = '<a href="' . esc_url( get_term_link( $term, $term->taxonomy ) ) . '" title="' . esc_attr( single_term_title( '', false ) ) . '" rel="v:url" property="v:title">' . single_term_title( '', false ) . '</a>';
	...
	public function do_post_type_archive_items() {
		...
		if ( is_paged() )
			$this->items[] = '<a href="' . esc_url( get_post_type_archive_link( $post_type_object->name ) ) . '" title="' . esc_attr( post_type_archive_title( '', false ) ) . '" rel="v:url" property="v:title">' . post_type_archive_title( '', false ) . '</a>';
	...
	public function do_user_archive_items() {
		...
		if ( is_paged() )
			$this->items[] = '<a href="'. esc_url( get_author_posts_url( $user_id ) ) . '" title="' . esc_attr( get_the_author_meta( 'display_name', $user_id ) ) . '" rel="v:url" property="v:title">' . get_the_author_meta( 'display_name', $user_id ) . '</a>';
	...
	public function do_day_archive_items() {
		...
		$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . esc_attr( $month ) . '" rel="v:url" property="v:title">' . $month . '</a>';

		/* Add the day item. */
		if ( is_paged() )
			$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) . '" title="' . esc_attr( $day ) . '" rel="v:url" property="v:title">' . $day . '</a>';
	...
	public function do_search_items() {
		...
		if ( is_paged() )
			$this->items[] = '<a href="' . get_search_link() . '" title="' . esc_attr( sprintf( $this->args['labels']['search'], get_search_query() ) ) . '" rel="v:url" property="v:title">' . sprintf( $this->args['labels']['search'], get_search_query() ) . '</a>';
	...
	function do_term_parents( $term_id, $taxonomy ) {
		...
		while ( $term_id ) {
			...
			$parents[] = '<a href="' . get_term_link( $term, $taxonomy ) . '" title="' . esc_attr( $term->name ) . '" rel="v:url" property="v:title">' . $term->name . '</a>';
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'breadcrumb-trail' ), $post_id ) . '" rel="v:url" property="v:title">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'breadcrumb-trail' ), $post_id ) . '" rel="v:url" property="v:title">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'breadcrumb-trail' ), $post_id ) . '" rel="v:url" property="v:title">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';

				/* If using the %author% tag, add a link to the post author archive. */
				elseif ( '%author%' == $tag )
					$this->items[] = '<a href="' . get_author_posts_url( $post->post_author ) . '" title="' . esc_attr( get_the_author_meta( 'display_name', $post->post_author ) ) . '" rel="v:url" property="v:title">' . get_the_author_meta( 'display_name', $post->post_author ) . '</a>';
				...
				elseif ( '%category%' == $tag ) {
					...
					if ( $terms ) {
						...
						$this->items[] = '<a href="' . get_term_link( $term, 'category' ) . '" title="' . esc_attr( $term->name ) . '" rel="v:url" property="v:title">' . $term->name . '</a>';
					}
				}
			}
		}
...
class bbPress_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		if ( !empty( $post_type_object->has_archive ) && !bbp_is_forum_archive() )
			$this->items[] = '<a href="' . get_post_type_archive_link( bbp_get_forum_post_type() ) . '" rel="v:url" property="v:title">' . bbp_get_forum_archive_title() . '</a>';
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			$this->items[] = '<a href="' . bbp_get_topic_tag_link() . '" rel="v:url" property="v:title">' . bbp_get_topic_tag_name() . '</a>';
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit() )
				$this->items[] = '<a href="' . bbp_get_topic_permalink( $topic_id ) . '" rel="v:url" property="v:title">' . bbp_get_topic_title( $topic_id ) . '</a>';
		...
		elseif ( bbp_is_single_reply() ) {
			...
			if ( bbp_is_reply_edit() ) {
				$this->items[] = '<a href="' . bbp_get_reply_url( $reply_id ) . '" rel="v:url" property="v:title">' . bbp_get_reply_title( $reply_id ) . '</a>';
		...
		elseif ( bbp_is_single_user() || bbp_is_single_user_edit() ) {
			...
			if ( bbp_is_single_user_edit() ) {
				$this->items[] = '<a href="' . bbp_get_user_profile_url() . '" rel="v:url" property="v:title">' . bbp_get_displayed_user_field( 'display_name' ) . '</a>';
